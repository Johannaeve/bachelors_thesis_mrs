...
def params_NAA_Cr_Ch_lorentz(shift, center_freq, n_baseline, base):
    # good start parameters for inverted peaks, parallel call tries several ph0
    w_ppm = np.linspace(1,4.5,2000)
    w = ppm2freq(w_ppm, center_freq)
    ...
    p0 = (w0_NAA,gamma_NAA,amp_NAA, w0_Cr1,gamma_Cr1, amp_Cr1, w0_Cr2,gamma_Cr2,amp_Cr2, w0_Ch,gamma_Ch,amp_Ch, ph0,ph1)
    
    bounds = ([    w0_NAA-20, 1, 0.0,         w0_Cr1-20, 1, 0.0,       w0_Cr2-20, 1, 0.0,         w0_Ch-20, 1, 0.0,       -np.pi,     -1e-3*np.pi    ],  # lower
...
def fit_2_voigt_GABA(freq, spec_cor, shift, center_freq, peak_mask):
...
def fit_GABA_2steps_voigt(freq, spec, shift, center_freq, peak_mask):
    ...
    f2, r2, area2, popt2, crlbs = fit_2_voigt_GABA(freq, spec_cor, shift, center_freq, peak_mask)
