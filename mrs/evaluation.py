...
### SNR
def calculate_snr(ppm, spec, shift=0.0,peak_window=(2.7, 3.2),noise_windows=((1.0,1.5), (6.0, 6.5))):
    ...
    if not np.any(peak_mask): return np.nan
    ...
    for w in noise_windows:
        ...
        if nm.sum() >= 8:
            ...
            if np.isfinite(s) and s > 0: noise_std = s; break
    if not np.isfinite(noise_std) or noise_std <= 0:
        order = np.argsort(ppm); tail = order[int(len(ppm)*0.9):]
        nm = np.zeros_like(peak_mask); nm[tail] = True; nm &= ~peak_mask
        if nm.sum() >= 8: noise_std = np.nanstd(re[nm], ddof=1)
    return np.nan if (not np.isfinite(noise_std) or noise_std <= 0) else float(peak_height/noise_std)

def calculate_snr_diff(rec_g, shift=0.0,peak_window=(2.88, 3.05),noise_windows=((2.70, 2.80), (3.10, 3.25))):
    # SNR = peak_height / noise_std computed on the difference spectrum
    
    ppm  = np.asarray(rec_g["ppm"])
    spec = rec_g["spec_cor"] if "spec_cor" in rec_g else rec_g["spec"]
    re   = np.real(np.asarray(spec))

    lo, hi = peak_window[0] + shift, peak_window[1] + shift
    pm = (ppm >= min(lo, hi)) & (ppm <= max(lo, hi))
    if not np.any(pm):
        return np.nan
    peak_height = float(np.nanmax(np.abs(re[pm])))
    noise_std = np.nan
    for w in noise_windows:
        lo, hi = w[0] + shift, w[1] + shift
        nm = (ppm >= min(lo, hi)) & (ppm <= max(lo, hi))
    ...
    if not np.isfinite(noise_std) or noise_std <= 0:
        nm = ~pm
        if nm.sum() >= 8:
            noise_std = np.nanstd(re[nm], ddof=1)
...
def plot_fit(ppm,spec,fit,baseline,residual,offset,peak1,peak2,peak3,title,save,ax=None,legend=True):
    ...
    ax.plot(ppm, spec, label="Data", linewidth=1, color='black')
    ax.plot(ppm, baseline, '--', linewidth=1, label="Baseline", color='grey')
    ax.plot(ppm, fit, '--', linewidth=1, label="Fit", color='mediumblue')

    ax.plot(ppm[peak1], residual[peak1]+offset, linewidth=1, label="Residual", color='deepskyblue')
    ax.plot(ppm[peak2], residual[peak2]+offset, linewidth=1, color='deepskyblue')
    ax.plot(ppm[peak3], residual[peak3]+offset, linewidth=1, color='deepskyblue')
    ...
    ax.axhline(offset, color='black', linestyle='--', linewidth=1)
